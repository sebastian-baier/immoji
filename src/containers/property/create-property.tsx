              ...
              {stepper.all.map((step, index, array) => (
                <React.Fragment key={step.id}>
                  ...
                  {index < array.length - 1 && (
                    <Separator
                      className={`flex-1 ${
                        index < stepper.current.index ? 'bg-purple-900' : 'bg-neutral-100'
                      }`}
                    />
                  )}
                </React.Fragment>
...
function ShippingComponent() {
  useWatch({ name: 'type' });
  const { register, getValues, setValue } = useFormContext<ShippingFormValues>();

  return (
    <div className="grid grid-cols-3 gap-4 place-items-stretch" {...register('type')}>
      <Card
        className={`flex flex-col h-60 cursor-pointer hover:border-purple-900
      ...
      >
        ...
        <p className="text-m self-center">Garage</p>
      ...
      <Card
        className={`flex flex-col h-60 hover:border-purple-900 cursor-pointer
      ...
      >
        ...
        <p className="text-m self-center">Wohnung</p>
      ...
      <Card
        ...
        onClick={() => {
          console.log('ho');
          setValue('type', PropertyTypes.HOUSE);
        }}
      >
        <Icons.home className="self-center fill-purple-900" size={8} />
        <p className="text-m self-center">Haus</p>
